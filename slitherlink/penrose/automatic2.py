...
num_puzzles_to_make = 50
starting_tiles = random.sample(tiles_list,num_puzzles_to_make)
num_ambiguous_tiles,num_starting_edges = 35, len(edges) # base = 5, divisions = 2
#num_ambiguous_tiles,num_starting_edges = 40, len(edges) # base = 5, divisions = 3
#num_ambiguous_tiles,num_starting_edges = 50, len(edges) # base = 5, divisions = 3

dirname = 'Divisions'+str(divisions)+'_'+'Base'+str(base)+'_'+'Seed'+str(seed)+\
                                     '_'+'Ambiguous'+str(num_ambiguous_tiles)

mkdir('base5_divisions3_files/'+dirname)


for i,starting_tile in enumerate(starting_tiles):
    subdir = 'base5_divisions3_files/'+dirname+'/'+str(i+1)
    print(subdir)
    mkdir(subdir)
